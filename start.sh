...
if [ ! -f ".env" ]; then
    ...
    echo ""
    echo -e "  3. 获取 API Key: ${BLUE}https://fm.duu.men/register?aff=H6wr${NC}"
    echo ""
    exit 1
fi
...
if [ -z "$ANTHROPIC_AUTH_TOKEN" ] || [ "$ANTHROPIC_AUTH_TOKEN" = "your_api_key_here" ]; then
    ...
    echo -e "  获取地址: ${BLUE}https://fm.duu.men/register?aff=H6wr${NC}"
    echo ""
    exit 1
fi
...
if [ -z "$ANTHROPIC_BASE_URL" ]; then
    ...
    export ANTHROPIC_BASE_URL="https://fm.duu.men"
fi
